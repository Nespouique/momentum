 ...
 *
 * Ce script:
 * 1. Lit les performances et sessions_notes de la base SQLite Hercules
 ...
 *
 * Usage: npx tsx scripts/hercules-import-history.ts
 */
...
const WORKOUT_MAPPING_PATH = path.join(__dirname, "hercules-workout-mapping.json");
...
// Performances à ignorer basées sur les notes d'écart
// Format: { date (ms timestamp), exerciseHerculesIds: [] to ignore }
// Notes matched by date from Hercules:
// - 23/04/2024 1713875644861: Pecs - "incliné au lieu de couché"
// - 07/05/2024 1715089116794: Pecs - "incliné au lieu de couché"
// - 20/08/2024 1724157417464: Pecs - "presse machine" (keeping - same exercise, just machine variation)
// - 25/08/2024 1724580824334: Fessiers - "presse horizontale au lieu de Split"
// - 28/10/2025 1761665050204: Pecs - "Haltères" (dumbbells instead of barbell)
const PERFORMANCES_TO_IGNORE: Array<{
  date: number;
  herculesExerciseIds: string[];
  reason: string;
}> = [
  {
    date: 1713875644861, // 23/04/2024
    herculesExerciseIds: ["134"], // Développé couché
    reason: "incliné au lieu de couché",
  },
  {
    date: 1715089116794, // 07/05/2024
    herculesExerciseIds: ["134"], // Développé couché
    reason: "incliné au lieu de couché",
  },
  {
    date: 1724580824334, // 25/08/2024
    herculesExerciseIds: ["270"], // Split squat
    reason: "presse horizontale au lieu de Split",
  },
  {
    date: 1761665050204, // 28/10/2025
    herculesExerciseIds: ["134"], // Développé couché
    reason: "Haltères (poids très différents)",
  },
];
...
async function main() {
  console.log("=== Story 4.3: Import de l'historique Hercules ===\n");
  console.log(`Database: ${DATABASE_URL.replace(/:[^:@]+@/, ":***@")}`);
  console.log(`Hercules DB: ${HERCULES_DB_PATH}\n`);

  // Load mappings from Stories 4.1 and 4.2
  ...
  try {
    ...
    // 1. Get all performances from Hercules
    ...
    for (const key of sortedKeys) {
      ...
      const existingSession = await pool.query(
        ...
        [
          ...
          new Date(firstPerf.date - 60000), // 1 minute before
          new Date(firstPerf.date + 60000), // 1 minute after
        ]
      );
      ...
      for (const perf of perfs) {
        // Check if this performance should be ignored
        const shouldIgnore = PERFORMANCES_TO_IGNORE.some(
          (ignore) =>
            Math.abs(ignore.date - perf.date) < 86400000 && // within 1 day
            ignore.herculesExerciseIds.includes(perf.exerciseId)
        );
        ...
        if (!exerciseMap) {
          // Exercise not mapped (probably ignored like stretches)
          exercisesSkipped++;
          continue;
        }

        // Find the corresponding WorkoutItemExercise for this exercise
        ...
        for (let i = 0; i < numSets; i++) {
          ...
          await pool.query(
            ...
            [
              ...
              completedAt, // Use session completedAt for all sets
            ]
          );
          sessionSetCount++;
          setsCreated++;
