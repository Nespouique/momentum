...
## Tasks / Subtasks
- [x] Étendre modèle User (AC: 1)
  - [x] Ajouter migration Prisma pour nouveaux champs
  - [x] Champs: age (Int?), height (Float?), goalDescription (String?)
  - [x] Générer Prisma Client
- [x] Créer endpoints profile (AC: 2)
  - [x] GET /profile - retourner profil utilisateur courant
  - [x] PATCH /profile - mettre à jour les champs profil
  - [x] PUT /profile/password - changer le mot de passe
  - [x] Validation avec Zod pour chaque endpoint
- [x] Créer page Profile Hub (AC: 3)
  - [x] Créer `app/(app)/profile/page.tsx`
  - [x] Afficher avatar placeholder, nom, email
  - [x] Liste de liens: Mensurations, Tracking Config, Edit Profile, Settings
  - [x] Bouton Logout
- [x] Créer formulaire édition profil (AC: 4)
  - [x] Créer `app/(app)/profile/edit/page.tsx`
  - [x] Champs: name, age, height, goalDescription
  - [x] Validation avec Zod
  - [x] Pré-remplir avec données actuelles
- [x] Créer formulaire changement password (AC: 5)
  - [x] Section dans la page Edit Profile
  - [x] Champs: currentPassword, newPassword, confirmNewPassword
  - [x] Validation: nouveau password différent, confirmation match
- [x] Implémenter feedback visuel (AC: 6)
  - [x] Installer composant Sonner pour notifications
  - [x] Afficher loading state pendant sauvegarde
  - [x] Toast success après sauvegarde réussie
  - [x] Afficher erreurs inline si échec
- [x] Ajouter navigation (AC: 7)
  - [x] Icône User dans bottom nav (déjà présent depuis Story 1.3)
...
## Testing
- Tester GET /profile retourne les données utilisateur
- Tester PATCH /profile met à jour les champs
- Tester PUT /profile/password avec bon current password
- Tester PUT /profile/password avec mauvais current password → erreur
- Tester que le Toast s'affiche après sauvegarde
...
### Completion Notes
- Extended User model with age, height, goalDescription fields
- Prisma migration `add_profile_fields` applied
