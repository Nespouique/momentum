# Momentum Companion - Product Requirements Document (PRD)
...
|------|---------|-------------|--------|
| 2026-02-05 | 1.0 | Creation initiale | John (PM) |
...
- **FR11**: L'API Momentum expose un endpoint bulk `/api/v1/health-sync` qui recoit les donnees de sante
- **FR12**: L'API Momentum cree automatiquement les TrackableItems systeme (pas, activite, calories, sommeil) s'ils n'existent pas
- **FR13**: L'API Momentum stocke les activites/entrainements Health Connect dans un modele dedie `HealthActivity`
- **FR14**: Les DailyEntry creees par la sync sont marquees `source: "health_connect"` pour les distinguer des saisies manuelles
...
- Les modifications API Momentum restent dans le monorepo existant
...
### Tech Stack - Modifications API Momentum
...
- **API** : Tests d'integration pour les endpoints health-sync (existant pattern Momentum)
...
### 5.1 Modifications Prisma (Momentum API)

#### Nouveau modele : `HealthActivity`
...
#### Modification du modele `DailyEntry` (prevu Epic 3, pas encore implemente)

Ajouter un champ `source` au modele DailyEntry planifie :
...
### 6.1 Nouveau endpoint : `POST /api/v1/health-sync`
...
### 6.2 Nouveau endpoint : `GET /api/v1/health-sync/status`
...
### 6.3 Nouveau endpoint : `GET /api/v1/health-activities`
...
### Epic 1 : API Momentum - Endpoints Health Sync

Ajouter les modeles de donnees et endpoints dans l'API Momentum existante pour recevoir et stocker les donnees de sante synchronisees depuis le companion.

### Epic 2 : Android Companion - Foundation & Sync

Creer l'app Android companion avec la configuration, les permissions Health Connect, la logique de sync en background, et l'ecran dashboard.
...
#### Story 1.1 : Schema de donnees Health Sync
...
#### Story 1.2 : Endpoint POST /api/v1/health-sync
...
#### Story 1.3 : Endpoints status et activites
